...
没有路由器时，服务器设备可以使用手机流量并开启热点，再让访问设备连接该热点。此时不要查找手机的公网流量 IP，而是在访问设备的 Wi-Fi 详情中查看：

- “IP 地址”：访问设备 IP，填写到白名单。
- “路由器”或“IPv4 默认网关”：服务器设备的热点 IP，用于打开 `http://这个地址:8000`。

例如访问设备显示 IP 地址 `192.168.43.26`、路由器 `192.168.43.1`，应将 `192.168.43.26` 加入白名单，并访问 `http://192.168.43.1:8000`。安全向导和后端区域都内置了这段折叠教程。
...
- IP 可能在设备重连 Wi-Fi 或重新连接热点后变化，需要重新确认并生成配置。
